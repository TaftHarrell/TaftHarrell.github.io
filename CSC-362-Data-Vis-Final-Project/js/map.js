...
hurricaneCounter(startYear, endYear, hurricaneData) {
  ...
  hurricaneData.forEach(h => {
    ...
    // Only count hurricanes within the year range and if the ID hasn't been counted
  ...
  // Convert yearCounts to array and sort by count descending
  const sortedYearCounts = Object.entries(yearCounts)
    .map(([year, count]) => ({ year, count }))
    .sort((a, b) => b.count - a.count);

  // Get the top 5 years with most hurricanes
  const top5Years = sortedYearCounts.slice(0, 5);

  return {
    totalHurricanes: hurricanes.size,
    top5Years: top5Years
  };
}
