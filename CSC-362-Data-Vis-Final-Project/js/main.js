...
d3.json('data/states-10m.json')
  .then(mapData => {
    ...
    d3.json("data/hurricanes.json").then(hurricaneData => {
      ...
      //default view
      document.getElementById("yearRank").textContent = "Top 5 years with the most hurricanes: 2005: 31, 2020: 31, 1978: 24, 1969: 23, 1975: 23";
...
document.getElementById('updateTimeRange').addEventListener('click', () => {
  ...
  let result = geoMap1.hurricaneCounter(startYear,endYear,hurricanes);
  
  document.getElementById("HurricaneCount").
            textContent="Total hurricanes from " + startYear + " - " + endYear + ": " + result.totalHurricanes + " hurricanes";  
            
          
  // Now safely access result.top5Years inside the same function
  const yearRank = result.top5Years;

  let yearRankText = "Top 5 years with the most hurricanes: ";
  yearRank.forEach(e => {
    yearRankText += `${e.year}: ${e.count}, `;
  });

  document.getElementById("yearRank").textContent = yearRankText.substring(0, yearRankText.length - 2);
